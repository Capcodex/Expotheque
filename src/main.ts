...
function parseCSV(text: string): EventRow[] {
  ...
  while (i < text.length) {
    ...
    if (inQuotes) {
      if (c === '"') {
        if (text[i+1] === '"') { field += '"'; i += 2; continue; } // échappement ""
...
function between(dt: Date | null, start?: Date | null, end?: Date | null): boolean {
  if (!dt) return false;
...
function renderCard(ev: EventRow): HTMLElement {
  ...
  if (!datesDisplay) {
    if (start && end) datesDisplay = `Du ${fmtShort(start)} au ${fmtShort(end)}`;
    else if (start)   datesDisplay = `À partir du ${fmtShort(start)}`;
    else if (end)     datesDisplay = `Jusqu’au ${fmtShort(end)}`;
  }
...
function populateCityFilter(events: EventRow[]) {
...
function classifyAndSort(events: EventRow[]) {
  const todayUTC = new Date(Date.UTC(today.getFullYear(), today.getMonth(), today.getDate(), 12, 0, 0));
...
function renderLists(events: EventRow[]) {
  ...
  ( $('#current-empty') as HTMLElement ).hidden = current.length > 0;
  ( $('#upcoming-empty') as HTMLElement ).hidden = upcoming.length > 0;
}

/** Applique les filtres UI */
function applyFilters(allEvents: EventRow[]) {
  const qInput = document.getElementById('q') as HTMLInputElement;
  const citySel = document.getElementById('city') as HTMLSelectElement;
...
async function init() {
  ...
  try {
    ...
    const txt = (await res.text()).replace(/^\uFEFF/, ''); // retire BOM
